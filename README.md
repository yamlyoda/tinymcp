...
### Цифры прогона симулятора (базовый прогон 2026-08-21)

Эндпоинт `/api/v1/orders/{order_id}/price`. Период «до» — засеянный здоровый
baseline до деплоя (4 000 запросов); «после» — живой прогон с деградацией.

| Период | Запросов | avg_ms | p95_ms | Hit-rate кэша |
|--------|----------|--------|--------|---------------|
| До (baseline)    | 4000 | **9.2**   | **29.2**  | **85.0%** |
| После (деплой)   | 4800 | **174.7** | **330.4** | **0.0%**  |

Поминутная разбивка прогона (деградация нарастает, кэш не попадается):

| Минута | Запросов | avg_ms | p95_ms | Hit-rate |
|--------|----------|--------|--------|----------|
...
Итого по прогону: avg вырос **в ~19 раз** (9.2 → 174.7 мс), p95 — **в ~11 раз**
